...
phase_install_tools() {
  ...
  if ! command -v xray-knife &>/dev/null; then
    ...
    local latest
    latest=$(curl -fsSL "https://api.github.com/repos/lilknighthood/xray-knife/releases/latest"       | jq -r '.assets[].name'       | grep -E "linux.*${arch}"       | grep -v ".sha"       | head -1)
    if [[ -n "$latest" ]]; then
      local dl_url
      dl_url=$(curl -fsSL "https://api.github.com/repos/lilknighthood/xray-knife/releases/latest"         | jq -r ".assets[] | select(.name == "${latest}") | .browser_download_url")
      curl -fsSL "$dl_url" -o /tmp/xray-knife.zip
      unzip -o /tmp/xray-knife.zip -d /tmp/xray-knife-tmp/
      ...
      warn "xray-knife not found — skipping (E2E test will be skipped)"
    fi
  else
    ok "xray-knife ready"
  fi
