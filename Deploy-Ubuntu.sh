...
maybe_screen() {
  ...
  if [[ "${ans,,}" == "y" ]]; then
    ...
    exec screen -S xhttp -c /dev/null bash "$0" "$@"
  fi
}
...
phase_install_tools() {
  ...
  # Remove any broken third-party repos before update
  for f in /etc/apt/sources.list.d/*.list /etc/apt/sources.list.d/*.sources; do
    [[ -f "$f" ]] || continue
    if grep -qi "packagecloud\|perfops\|p1/" "$f" 2>/dev/null; then
  ...
  apt-get install -y curl jq unzip screen uuid-runtime socat -qq
  ...
  if ! command -v xray-knife &>/dev/null; then
    ...
    local latest
    latest=$(curl -fsSL "https://api.github.com/repos/lilknighthood/xray-knife/releases/latest" | jq -r '.tag_name')
    local arch
    arch=$(uname -m)
    [[ "$arch" == "x86_64" ]] && arch="amd64"
    [[ "$arch" == "aarch64" ]] && arch="arm64"
    local url="https://github.com/lilknighthood/xray-knife/releases/download/${latest}/xray-knife-linux-${arch}"
    curl -fsSL "$url" -o /usr/local/bin/xray-knife
